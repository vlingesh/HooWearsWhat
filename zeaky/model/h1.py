...
parser.add_argument('--nfeat', type=int, help='number features', default=1)
parser.add_argument('--cat', type=str, help='cat or elt-sum', default='False')
parser.add_argument('--bias', type=bool, default=True)
args = parser.parse_args()
def str2bool(str1):
  str1 = str(str1).lower()
  if "false" in str1 or "0" in str1:
    return False
  else:
    return True
args.cat = str2bool(args.cat)
tmp_dir = 'tmp'
...
def make_net():
  ...
  fpath = join(tmp_dir, "h1feat%d_test.pt"%args.nfeat)
  with open(fpath, 'w') as f:
...
def make_solver():
  ...
  sp['display'] = '10'
  sp['snapshot'] = '2000'
  sp['snapshot_prefix'] = '"snapshot/h1feat%d"'%args.nfeat
  sp['gamma'] = '0.1'
  sp['max_iter'] = '40000'
  sp['solver_mode'] = 'GPU'
  fpath = join(tmp_dir, "h1feat%d_solver.pt"%args.nfeat)
  f = open(fpath, 'w')
